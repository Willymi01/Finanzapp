...
export default function Dashboard({ state }) {
  ...
  return <>
    <div className="metric-grid">
      ...
      <MetricCard label="Kaufziel" value={euro(state.project.purchasePrice)} hint={`Ziel: ${new Date(`${state.project.target}T12:00:00`).toLocaleDateString('de-DE',{month:'long',year:'numeric'})}`}/>
    ...
    <div className="content-grid">
      ...
      <Panel title="Heute" subtitle={currentLabel} className="span-4">
        <div className="coach">
          ...
          Noch fehlen rechnerisch <b>{euro(gap)}</b> bis zum Ziel.
        </div>
      </Panel>
      <Panel title="Aktueller Vermögensstand" subtitle="Diese Werte stammen aus deinen Eingaben oder dem letzten Zwischenstand" className="span-12">
        <div className="asset-breakdown">
          ...
          <div><span>Gesamt inkl. Notgroschen</span><b>{euro(state.assets.home+state.assets.pension+state.assets.emergency)}</b></div>
        </div>
      </Panel>
